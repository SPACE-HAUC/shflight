...
#define DATA_SIZE_MAX (PACKET_SIZE - sizeof(sw_upd_data_t))

#define MAX_SEND_ATTEMPTS 5
#define MAX_RECV_ATTEMPTS 100

#define GS_SLEEP_TIME 1 // 0.1 s
#define SH_SLEEP_TIME 1 // 0.1 s
...
static int checksum_md5(const char *fname, char *hash, ssize_t hashlen)
{
    ...
    if (fname == NULL)
    {
        eprintf("No valid file specified");
        retval = -1;
        goto hash_exit;
    }
    if (hash == NULL)
    {
        eprintf("hash pointer is null");
        retval = -1;
        goto hash_exit;
    }
    if (hashlen < 32)
    {
        eprintf("Not enough memory for hash storage");
        retval = -1;
        goto hash_exit;
    }
    ...
    if (rdsz < sizeof(buf))
    {
        eprintf("Hash size read %d bytes instead of %d bytes", rdsz, (int)sizeof(buf));
        retval = -1;
        goto hash_cleanup;
    }
