...
void * acs(void * id)
{
    do {
        ...
        switch(g_program_state){
            ...
            int FiringTime = COARSE_TIME_STEP - COARSE_MEASURE_TIME - MAG_MEASURE_TIME; // time allowed to fire
            ...
            time_on = time_on > SUNPOINT_DUTY_CYCLE ? SUNPOINT_DUTY_CYCLE : time_on ; // safety measure
            int time_off = SUNPOINT_DUTY_CYCLE - time_on ;
            int FiringTime = FINE_TIME_STEP - FINE_MEASURE_TIME - MAG_MEASURE_TIME ; // time allowed to fire
            DECLARE_VECTOR(fire, int) ; 
            z_fire = 1 ; // z direction is the only direction of fire
            while(FiringTime > 0){
                HBRIDGE_ENABLE(fire);
